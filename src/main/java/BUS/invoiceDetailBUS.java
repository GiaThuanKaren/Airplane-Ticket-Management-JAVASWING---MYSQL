...
public class invoiceDetailBUS {
    ...
    private typeBUS typBUS;
    private ArrayList<typeDTO> typList;
    ...
    public long sotien(int sl, String typeId) {
        ...
        typList = typBUS.getList();
        int price = 1;
        for(typeDTO typ : typList) {
            if (typ.getTypeID().equals(typeId))
            {
                price = (int) typ.getTypePrice();
            }
        }
        return price*sl;
    }
